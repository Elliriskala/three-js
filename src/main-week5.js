...
async function init() {
  ...
  // Load landscape with proper floor mesh extraction
  const loader = new GLTFLoader();
  loader.load('./landscape/landscape.gltf', async function (gltf) {
    const root = gltf.scene;
    scene.add(root);

    // Try to find a mesh for physics - use the whole scene or find specific mesh
    const floor = root.getObjectByProperty('isMesh', true) || root;

    if (!floor) {
      console.error('No mesh found inside landscape.glb for physics!');
      return;
    }

    floor.receiveShadow = true;

    teleportgroup.add(root);
